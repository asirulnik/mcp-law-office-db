...
"""
Law Office Database Setup Script

This script initializes the SQLite database for the Law Office MCP Server with the
necessary schema, tables, indexes, triggers, and views for managing law office clients,
matters, case files, billing records, and invoices.

Usage:
    python setup_law_office.py [--db-path PATH]
"""
...
logger = logging.getLogger("law_office_setup")

# Path to the SQL schema file
SCHEMA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "database")
DEFAULT_DB_PATH = os.path.join(SCHEMA_DIR, "law_office.db")

def parse_args():
    """Parse command line arguments"""
    parser = argparse.ArgumentParser(description="Set up the Law Office SQLite database")
    parser.add_argument("--db-path", type=str, default=DEFAULT_DB_PATH,
                      help=f"Path to create/initialize the SQLite database (default: {DEFAULT_DB_PATH})")
    return parser.parse_args()

def read_schema():
    """Read the database schema from SQL file"""
    ...
    if not os.path.exists(schema_file):
        logger.info("Schema file not found, creating it from embedded schema...")
...
def initialize_database(db_path):
    ...
    if db_exists:
        ...
        if response != 'y':
            logger.info("Setup cancelled. Existing database preserved.")
            return False
    ...
    try:
        ...
        if os.path.exists(db_path) and not db_exists:
            # If an error occurred and we created a new file, clean it up
            os.remove(db_path)
            logger.info(f"Removed incomplete database file: {db_path}")
        return False
...
def add_sample_data(conn):
    ...
    cursor.execute("""
        ...
        VALUES ('Acme Corporation', 'John Smith, john@acme.com, 555-123-4567')
    """)
    client_id = cursor.lastrowid
    
    # Add a sample matter
    cursor.execute("""
        INSERT INTO matters (client_id, matter_name, matter_status)
        VALUES (?, 'Contract Negotiation', 'Open')
    """, (client_id,))
    matter_id = cursor.lastrowid
    
    # Add some sample case file entries
    cursor.execute("""
        INSERT INTO case_file_entries (matter_id, type, date, title, from_party, to_party, content, synopsis)
        VALUES (?, 'Email', datetime('now', '-2 days'), 'Initial Discussion', 'John Smith', 'Our Firm', 
                'We would like to engage your services for contract review and negotiation with our supplier.',
                'Client requested contract review services')
    """, (matter_id,))
    entry_id1 = cursor.lastrowid
    
    cursor.execute("""
        INSERT INTO case_file_entries (matter_id, type, date, title, from_party, to_party, content, synopsis)
        VALUES (?, 'Note', datetime('now', '-1 day'), 'Phone Conference', 'Attorney', 'Internal', 
                'Discussed contract terms with client. Key issues include delivery schedules and payment terms.',
                'Initial call to discuss contract concerns')
    """, (matter_id,))
    entry_id2 = cursor.lastrowid
    
    # Add a sample billing entry
    cursor.execute("""
        INSERT INTO billing_entries (
            matter_id, substantiating_entry_id_1, billing_category, 
            billing_start, billing_stop, billing_hours, billing_description, status
        )
        VALUES (?, ?, 'Legal Consultation', 
                datetime('now', '-1 day', '-1 hour'), datetime('now', '-1 day'), 
                1.0, 'Initial review of contract terms and client consultation', 'unbilled')
    """, (matter_id, entry_id2))
    
    conn.commit()
    logger.info("Sample data added successfully.")

# Embedded schema SQL (used if schema.sql doesn't exist)
SCHEMA_SQL = """-- Clients Table
